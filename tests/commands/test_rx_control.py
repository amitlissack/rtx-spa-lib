...
@pytest.mark.parametrize(
    ...
    argvalues=[[0, bytes([0x0D, 0x0C])], [1, bytes([0x1D, 0x1C])]],
)
def test_channel(
...
@pytest.mark.parametrize(
    ...
    argvalues=[[True, bytes([0x0D, 0x0C])], [False, bytes([0x4D, 0x4C])]],
)
def test_parity(
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        [RtxArincSpeed.low, bytes([0x8D, 0x8C])],
        [RtxArincSpeed.high, bytes([0x0D, 0x0C])],
    ],
)
def test_speed(
